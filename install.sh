...
#  出口一致版
# 每运行一次只新增一个国家；所有  国家共享 UUID/REALITY 参数，因此手机换国家只改端口。
# 每条  线路只有一个代理出口（SOCKS5/HTTP），不配置 DIRECT/fallback。
...
APP_DIR="/usr/local/etc/xray/multi-exit"
...
bootstrap_dependencies(){
  ...
  command -v qrencode >/dev/null 2>&1 || install_pkg qrencode || true
  command -v systemctl >/dev/null 2>&1 || die "系统没有 systemd/systemctl，当前通用版暂不支持。"
}
...
install_xray_if_needed(){
  ...
  progress 4 "未检测到 Xray，正在自动安装..."
  local installer="/tmp/v11-xray-install.sh"
  curl -fL --retry 3 --connect-timeout 10 --max-time 60 \
    "https://github.com/XTLS/Xray-install/raw/main/install-release.sh" \
    -o "${installer}" || die "下载 Xray 官方安装脚本失败。"
  chmod +x "${installer}"

  bash "${installer}" install >/tmp/v11-xray-install.log 2>&1 || {
    cat /tmp/v11-xray-install.log 2>/dev/null || true
    die "Xray 自动安装失败。"
  }

  XRAY_BIN="$(command -v xray || true)"
  [[ -n "${XRAY_BIN}" && -x "${XRAY_BIN}" ]] || XRAY_BIN="/usr/local/bin/xray"
  [[ -x "${XRAY_BIN}" ]] || die "Xray 安装后仍未找到可执行文件。"
}

ensure_xray_base_config(){
...
try:
    ...
    raise SystemExit(f"现有 Xray 配置不是有效 JSON：{e}")
PY
    return 0
  fi

  progress 6 "创建全新 Xray 基础配置..."
  cat > "${XRAY_CONF}" <<'JSON'
{
  "log": {
    "loglevel": "warning"
...
if ! "${XRAY_BIN}" run -test -config "${XRAY_CONF}" >/tmp/v11-base-test.log 2>&1; then
  cat /tmp/v11-base-test.log 2>/dev/null || true
  die "现有/新建 Xray 基础配置检查失败，已停止，不会继续写入。"
fi
...
say "${C_CYAN}${C_BOLD}        多国家动态IP · 出口一致通用版${C_RESET}"
say "${C_CYAN}${C_BOLD}══════════════════════════════════════════════════${C_RESET}"
say "${C_BLUE}每次运行只新增一个国家；代理是该线路唯一公网出口。${C_RESET}"
say "${C_BLUE}同一台服务器所有  国家共用手机认证参数，换国家只改端口。${C_RESET}"
...
PY
then
  die "已经存在同名  线路「${LINE_NAME}」，请使用其他备注。"
fi
...
IN_TAG="country-in-${SAFE_ID}"
OUT_TAG="country-out-${SAFE_ID}"
BACKUP="${XRAY_CONF}.backup.multi-exit.$(date +%Y%m%d-%H%M%S)"
cp "${XRAY_CONF}" "${BACKUP}"
...
if not rules: raise SystemExit("找不到该  路由规则")
if rules[0].get("outboundTag") != out_tag:
    raise SystemExit(" 第一条专属规则不是唯一代理出口")
for r in rules:
    t=str(r.get("outboundTag","")).lower()
    if t in {"direct","freedom"} or "fallback" in t:
        raise SystemExit("检测到该  线路存在 DIRECT/fallback")
print("_ROUTE_OK")
PY

if ! "${XRAY_BIN}" run -test -config "${XRAY_CONF}" >/tmp/multi-exit-xray-test.log 2>&1; then
  cp "${BACKUP}" "${XRAY_CONF}"
  cat /tmp/multi-exit-xray-test.log
...
TMP_CLIENT="/tmp/multi-exit-client-${SAFE_ID}.json"
cat > "${TMP_CLIENT}" <<EOF
{
...
"${XRAY_BIN}" run -config "${TMP_CLIENT}" >/tmp/multi-exit-client-${SAFE_ID}.log 2>&1 &
CLIENT_PID=$!
...
say "同一台服务器后续新增的  国家会复用相同 UUID / REALITY 参数。"
say "手机换国家时只改成对应国家的 VLESS 端口。"
say "线路登记：${C_MAGENTA}${REGISTRY_FILE}${C_RESET}"
