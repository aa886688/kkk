...
if [[ ! -f "$MASTER_FILE" ]]; then
  ...
  PUB="$(printf "%s\n" "$KEYS" | awk -F': ' '/^Password:/{print $2;exit}')"
  [[ -n "$PUB" ]] || PUB="$(printf "%s\n" "$KEYS" | awk -F': ' '/Password \(PublicKey\):/{print $2;exit}')"
  [[ -n "$PUB" ]] || PUB="$(printf "%s\n" "$KEYS" | awk -F': ' '/PublicKey:/{print $2;exit}')"
  [[ -n "$PUB" ]] || die "无法从 xray x25519 输出中读取 REALITY Password/PublicKey。"
  SID="$(python3 - <<'PY'
import secrets
...
settings={"address":host,"port":int(pport)}
if user or pw:
    settings["user"]=user
    settings["pass"]=pw
c.setdefault("inbounds",[]).append({"tag":it,"listen":"0.0.0.0","port":int(port),"protocol":"vless","settings":{"users":[{"id":uuid,"flow":"xtls-rprx-vision"}],"decryption":"none"},"streamSettings":{"method":"raw","security":"reality","realitySettings":{"show":False,"target":f"{sni}:443","xver":0,"serverNames":[sni],"privateKey":priv,"shortIds":[sid]}}})
c.setdefault("outbounds",[]).append({"tag":ot,"protocol":proto,"settings":settings})
...
cat > "$CLIENT" <<EOF2
{"inbounds":[{"listen":"127.0.0.1","port":${LOCAL},"protocol":"socks","settings":{"udp":false}}],"outbounds":[{"protocol":"vless","settings":{"address":"127.0.0.1","port":${PORT},"id":"${UUID}","encryption":"none","flow":"xtls-rprx-vision"},"streamSettings":{"method":"raw","security":"reality","realitySettings":{"fingerprint":"chrome","serverName":"${SNI}","password":"${PUBLIC_KEY}","shortId":"${SHORT_ID}","spiderX":"/"}}}]}
EOF2
