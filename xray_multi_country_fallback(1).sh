...
read -rp "手机连接的 VLESS 端口（例如 8444）: " VLESS_PORT
read -rp "本机 fallback SOCKS 端口（例如 1089）: " LOCAL_PORT
[[ "$VLESS_PORT" =~ ^[0-9]+$ ]] || { echo "VLESS 端口格式错误"; exit 1; }
[[ "$LOCAL_PORT" =~ ^[0-9]+$ ]] || { echo "本机 SOCKS 端口格式错误"; exit 1; }

read -rp "VLESS UUID（直接回车自动复用现有 VLESS UUID）: " UUID_INPUT
read -rp "上游 SOCKS5 Host: " UP_HOST
read -rp "上游 SOCKS5 Port: " UP_PORT
read -rp "上游用户名: " UP_USER
read -rsp "上游密码: " UP_PASS
echo
[[ "$UP_PORT" =~ ^[0-9]+$ ]] || { echo "上游端口格式错误"; exit 1; }

if [[ -z "$UUID_INPUT" ]]; then
  UUID_INPUT="$(python3 - <<'PY'
import json
